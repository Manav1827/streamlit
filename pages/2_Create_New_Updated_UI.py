...
st.set_page_config(page_title="🎨 Invitation & Reel Creator", layout="wide")

# ----------------- MAIN HEADER -----------------
st.markdown("""
    <h1 style='text-align: center; color: #4f46e5;'>
    ✨ Create <span style="color:#9333ea;">Something Amazing</span>
    </h1>
    <p style='text-align: center; color: #6b7280;'>
    Choose your project type and bring your creative vision to life with our powerful design tools.
    </p>
""", unsafe_allow_html=True)
...
with col1:
    st.markdown("""
    <div style='border-radius:15px; border: 1px solid #e5e7eb; padding: 20px; background-color:white; box-shadow:0px 4px 12px rgba(0,0,0,0.1);'>
        <h3>📑 Birthday Invitation</h3>
        <p>Create stunning personalized birthday invitations with AI-powered backgrounds, custom text effects, and animated elements.</p>
        <ul>
            <li>AI-generated backgrounds</li>
            <li>Custom text effects & animations</li>
            <li>Photo integration & editing</li>
        </ul>
        
    </div>
    """, unsafe_allow_html=True)
    if st.button("Start ➡️", key="start_invite",):
        st.query_params.update({"page": "invitation"})
        st.rerun()    
...
with col2:
    ...
    <div style='border-radius:15px; border: 1px solid #e5e7eb; padding: 20px; background-color:white; box-shadow:0px 4px 12px rgba(0,0,0,0.1);'>
        <h3>🎥 Video Reel</h3>
        <p>Craft engaging video reels with multiple slides, personal photos, stickers, animations, and background music.</p>
        <ul>
            <li>Multi-slide video creation</li>
            <li>Background music & animations</li>
            <li>Custom stickers & positioning</li>
        </ul>
    
    </div>
    """, unsafe_allow_html=True)
    if st.button("Start ➡️", key="start_reel"):
        st.query_params.update({"page": "reel"})
        st.rerun()    
with col3:
    st.markdown("""
    <div style='border-radius:15px; border: 1px solid #e5e7eb; padding: 20px; background-color:white; box-shadow:0px 4px 12px rgba(0,0,0,0.1);'>
        <h3>🤖 AI Sample Generation</h3>
        <p>Generate AI-powered images based on any text prompt. Perfect for creative samples, thumbnails, or inspiration.</p>
        <ul>
    ...
    if st.button("Start ➡️", key="start_ai_sample"):
        st.query_params.update({"page": "ai_sample"})
        st.rerun()
...
if page == "invitation":
    ...
    if name and date and st.session_state.user:
    ...
    if "output_path" in st.session_state:
        ...
        with st.expander("🎨 Shadow & Duotone Effects"):
            shadow = st.selectbox(
                "Choose Shadow Effect", ["None", "drop_shadow", "glow", "outline"]
            )
            duotone = st.selectbox(
                "Choose Duotone Effect", ["None", "cherry_fuchsia", "grayscale"]
            )

            if st.button("Apply Effects"):
                final_effect_path = invitation_generator.apply_effects(
                    current_output, shadow.lower(), duotone.lower()
                )
                st.session_state["output_path"] = final_effect_path  # ✅ Update session
                st.success(f"✅ Effects applied. File saved at {final_effect_path}")
                st.image(final_effect_path, use_column_width=True)
    else:
        st.warning("⚠️ Please generate the invitation first before applying effects.")
                        ...
                        try:
                            ...
                            if os.path.exists(animated_path):
                                ...
                                if "user" in st.session_state:
                                    user_id = st.session_state.user["localId"]
                                    project_name = f"Invitation_{name}_{date}".replace(" ", "_")
                                    ...
                                    project_data = {
                                        ...
                                        "description": f"Invitation for {name} on {date}"
                                    }

                                    save_project(user_id, project_name, project_data)
                                    st.success("✅ Project saved to My Projects")
...
# ------------------ Basic Details -------------------
        ...
        if not text_slides:
            ...
            with st.spinner("Generating Reel... Please wait..."):
                try:
                    ...
                    if "user" in st.session_state:
                        user_id = st.session_state.user["localId"]
                        project_name = f"Reel_{name}_{date}".replace(" ", "_")
                        ...
                        project_data = {
                            ...
                            "description": f"Reel for {name} on {date}"
                        }
    ...
    if ai_prompt and st.session_state.user:
        user_id = st.session_state.user["localId"]
            ...
            with st.spinner("Generating image... Please wait..."):
                try:
                    ...
                    if generated_image_path and os.path.exists(generated_image_path):
                        ...
                        # ✅ Save final AI image project separately as completed
                        project_name = f"AI_Image_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                        project_data = {
                            "title": project_name,
                            "type": "ai_image",
                            "created_at": datetime.now().strftime('%Y-%m-%d %H:%M'),
                            ...
                            "description": f"AI-generated image using prompt: {ai_prompt}"
                        }
                        save_project(user_id, project_name, project_data)
                        st.success("✅ Project saved to My Projects!")
                    else:
